...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if(table[reg.ax].intercepted == 1){
		//some pids are monitored. have to check if current pid is
		...
		//all pids are being monitored for that syscall
		else{
			log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
...
/**
 * Checks that PID given is valid
 */
int checkValidPID(int pid){
	
    //check that pid is not negative
    if(pid < 0){
        return 1;
    }
	
    //check that pid is not 0 and that it belongs to a valid process
...
/**
 * Checks that given syscall is valid
 */
int checkSyscall(int syscall){
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	//check syscall for all commands
	
	//checks that syscall is valid for all commands
	if(checkSyscall(syscall)){
		
		return -EINVAL;
	}

	//go through each command to see what to do
	if(cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		//check that system call is not already intercepted
		...
		//check that system call is already intercepted. Otherwise it can't be deintercepted
		...
		//deintercept
			...
			//revert system call table back to normal.
			spin_lock(&sys_call_table_lock);
			set_addr_rw((unsigned long) sys_call_table);
			...
			spin_lock(&my_table_lock);
            //remove pid list for that syscall
            destroy_list(syscall);
			//monitored and intercept become 0
        ...
        if (current_uid() != 0) {
            ...
            //pid cannot be 0 if calling process is not root
            if (pid == 0) {

                return -EPERM;
            }

            //check if pid is owned by calling process
            else if (check_pids_same_owner(pid, current->pid) != 0) {

                return -EPERM;
            }

            //syscall has not been intercepted yet
            else if (table[syscall].intercepted == 0) {
		...
		//check if this is the first pid being monitored by this syscall
        ...
        if (current_uid() != 0) {
            ...
            //syscall has not been intercepted yet or pid is not being monitored
            else if (check_pid_monitored(syscall, pid) == 0 || table[syscall].intercepted == 0)  {
        ...
        //calling process is root
            ...
            //syscall has not been intercepted or pid is not being monitored
...
static int init_function(void) {
	...
	spin_lock(&sys_call_table_lock);
	//set system call table as writable to intercept chosen call
	set_addr_rw((unsigned long) sys_call_table);
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;
	sys_call_table[__NR_exit_group] = my_exit_group;
	//set call table back to read only after hijacking
	set_addr_ro((unsigned long) sys_call_table);
	spin_unlock(&sys_call_table_lock);
...
static void exit_function(void) {
    ...
    //set system call table as writable to restore original system call
	spin_lock(&sys_call_table_lock);
    set_addr_rw((unsigned long) sys_call_table);
    ...
    //restore _NR_exit_group to its original syscall
    ...
    //set system call table back to read only after restoring the original syscalls
    set_addr_ro((unsigned long) sys_call_table);
	spin_unlock(&sys_call_table_lock);
