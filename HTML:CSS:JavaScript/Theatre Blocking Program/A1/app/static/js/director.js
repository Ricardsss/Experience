	    ...
	    .then((jsonResult) => {
	    	...
	    	for (i = 1; i <= numberOfParts; i++) {
	    		...
	    		for (k = 1; k <= numberOfActors; k++) {
	    			actors.push(jsonResult.parts[i][k].actor)
	    			if (jsonResult.parts[i][k].block == 0) {
	    				blocks.push('')
	    			}
	    			else {
	    				blocks.push(jsonResult.parts[i][k].block)
	    			}
	    		}
	    		addBlockToScreen(lines, startChar, endChar, actors, blocks)
	    	}
...
function changeScript() {
    ...
    const scriptNumHolders = document.querySelectorAll("#scriptText")
    var scriptNums = []
    var i
    for (i = 0; i < scriptNumHolders.length; i++) {
        scriptNums.push(scriptNumHolders[i].value)
    }
    const numOfParts = blocks.children.length
    const numOfActors = scriptNums.length / numOfParts
    console.log(scriptNums)
    ...
    for (j = 0; j < numOfParts; j++) {
        parts[(j + 1).toString()] = []
        var l
        for (l = 0; l < numOfActors; l++) {
            parts[(j + 1).toString()].push(scriptNums[k])
            k++
        }
    }
    let data = {
